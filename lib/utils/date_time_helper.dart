...
class DateTimeHelper {
  ...
  String get createdAt =>
      DateFormat(defaultFormat, locale).format(DateTime.now());
  ...
  String convert(
      {required String raw,
      String source = defaultFormat,
      required String destination}) {
    try {
      return DateFormat(destination, locale)
          .format(DateFormat(source, locale).parse(raw));
    } catch (e) {
      return '$e';
    }
  }
  ...
  DateTime toDateTime(
      {required String dateTime, String format = defaultFormat}) {
    try {
